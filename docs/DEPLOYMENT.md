...
| Cog can't find any notes | `wiki-curator-cog` is missing the `corpus-reader` role, so `wcs.corpus.read` is denied | Check `identity_registry.MACHINES` in api-kaianolevine-com declares the role, and that boot reconciliation applied it. `GET /v1/identity/whoami` reports what the API resolves for this caller |
| Every call returns 401 | `WIKI_CURATOR_COG_API_KEY` unset, stale, or not matching the API's copy | Compare the Doppler value against the API's environment. There is no fallback credential — rotation must land on both sides |
